...
describe('감독 기록', () => {
  ...
  it('endMatch: 감독 승무패 세션 스탯 갱신', () => {
    ...
    expect(s.sessionStats.coachStats['선수1'].goalsFor).toBe(2);
    expect(s.sessionStats.coachStats['선수1'].goalsAgainst).toBe(1);
    expect(s.sessionStats.coachStats['선수2'].losses).toBe(1);
    expect(s.sessionStats.coachStats['선수2'].goalsAgainst).toBe(2);
  });
  ...
  it('getCoachStatsFromArchive: 누적 합산 (득실 포함)', () => {
    const sheet = ctx.store.ensureSheet('기록실_감독기록');
    sheet.push(['2026-01-01', '선수1', 'RED', 2, 1, 1, 0, 5, 3]);
    sheet.push(['2026-02-01', '선수1', 'RED', 1, 1, 0, 0, 2, 1]);
    const stats = ctx.api.getCoachStatsFromArchive();
    expect(stats['선수1'].games).toBe(3);
    expect(stats['선수1'].wins).toBe(2);
    expect(stats['선수1'].goalsFor).toBe(7);
    expect(stats['선수1'].goalsAgainst).toBe(4);
    expect(stats['선수1'].goalDifference).toBe(3);

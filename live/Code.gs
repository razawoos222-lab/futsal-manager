...
function syncTeamsForMode_(state) {
  ...
  getTeamNamesForMode(mode).forEach(name => {
    if (!state.teams[name] || typeof state.teams[name] !== 'object') {
      state.teams[name] = { players: [], goalkeeper: null, captain: null, viceCaptain: null };
    }
    if (!Array.isArray(state.teams[name].players)) {
      if (Array.isArray(state.teams[name])) {
        state.teams[name] = { players: state.teams[name], goalkeeper: null, captain: null, viceCaptain: null };
      } else {
...
function getAppState() {
  ...
  const buildInitialState_ = () => ({
    ...
    teams: {
      RED: { players: [], goalkeeper: null, captain: null, viceCaptain: null },
      BLUE: { players: [], goalkeeper: null, captain: null, viceCaptain: null },
      YELLOW: { players: [], goalkeeper: null, captain: null, viceCaptain: null },
      BLACK: { players: [], goalkeeper: null, captain: null, viceCaptain: null }
    },
    originalTeams: null,
    match: {
      count: 1,
...
function performTeamAllocation(algorithm, shuffleEnabled = false) {
  return safeExecute(() => {
    ...
    teamNames.forEach(name => {
      teams[name] = { players: [], goalkeeper: null, captain: null, viceCaptain: null };
    });
    ...
    getTeamNamesForMode(teamCount).forEach(name => {
      if (!finalTeams[name]) {
        finalTeams[name] = { players: [], goalkeeper: null, captain: null, viceCaptain: null };
      }
    });
